#ifndef SAP_GPU_LL_ITREE_PARALLEL_H_INCLUDED
...
//OPENCL
cl_platform_id sap_gpu_ll_platform;
cl_device_id sap_gpu_ll_device_id;
cl_context sap_gpu_ll_context;
cl_command_queue sap_gpu_ll_commands;
cl_program sap_gpu_ll_program;
cl_kernel sap_gpu_ll_kernel;
...
void build_sap_gpu_ll_parallel(int local_size,unsigned int total_objects)
{
...
//        replace(sap_gpu_ll_kernel_program,"VAR3",LL::to_string(local_size));
    ...
    //GPU CONF
    int err;
    const char* kernel_c_str=sap_gpu_ll_kernel_program.c_str();
    cl_platform_id sap_gpu_ll_platform_temp[2];
    cl_uint num;
    err = clGetPlatformIDs(2, sap_gpu_ll_platform_temp, &num);
    if(err < 0)
    {
        printf("Couldn't identify a platform");
        exit(1);
    }
    sap_gpu_ll_platform=sap_gpu_ll_platform_temp[num-1];
    //GET GPU DEVICE
    err = clGetDeviceIDs(sap_gpu_ll_platform, CL_DEVICE_TYPE_GPU, 1, &sap_gpu_ll_device_id, NULL);
    if (err != CL_SUCCESS)
    {
    ...
    char buf[4096];
    err = clGetDeviceInfo(sap_gpu_ll_device_id, CL_DEVICE_NAME, sizeof(buf), buf, NULL);
    if (err != CL_SUCCESS)
    {
    ...
    sap_gpu_ll_context = clCreateContext(0, 1, &sap_gpu_ll_device_id, NULL, NULL, &err);
    if (!sap_gpu_ll_context)
    {
        printf("Error: Failed to create a compute context!\n");
        exit(1);
    }
    sap_gpu_ll_commands = clCreateCommandQueue(sap_gpu_ll_context, sap_gpu_ll_device_id, 0, &err);
    if (!sap_gpu_ll_commands)
    {
        printf("Error: Failed to create a command commands!\n");
        exit(1);
    }
    sap_gpu_ll_program = clCreateProgramWithSource(sap_gpu_ll_context, 1, &kernel_c_str, NULL, &err);
    if (!sap_gpu_ll_program)
    {
    ...
    sap_gpu_ll_kernel = clCreateKernel(sap_gpu_ll_program, "sap_gpu_ll_parallel", &err);
    if (!sap_gpu_ll_kernel || err != CL_SUCCESS)
    {
        printf("Error: Failed to create compute kernel!\n");
        exit(1);
    }
    gpu_ll_results=new char[max_outputs];
}

void delete_sap_gpu_ll_parallel()
{
    if(sap_gpu_ll_kernel_program.size())
    {
        clReleaseProgram(sap_gpu_ll_program);
        clReleaseKernel(sap_gpu_ll_kernel);
        clReleaseCommandQueue(sap_gpu_ll_commands);
        clReleaseContext(sap_gpu_ll_context);
    }
    if(gpu_ll_results)
        delete[](gpu_ll_results);
    gpu_ll_results=nullptr;
}

void SAP_GPU_LL_ITree_Parallel(float* objects,
                               int* total_sizes,
                               ...
                               unsigned int local_size)
{
    ...
    ref_buffer = clCreateBuffer(sap_gpu_ll_context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, size_x*size_y * sizeof(int), total_sizes, &err);
    if(err < 0)
    {
        printf("Couldn't create reference buffer");
        return;
    }
    output_buffer= clCreateBuffer(sap_gpu_ll_context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR , max_outputs * sizeof(char), gpu_ll_results, &err);
    if(err < 0)
    {
...
//    err |= clSetKernelArg(sap_gpu_ll_kernel, 3, (local_size*(local_size-1)/2) * sizeof(char), NULL);
    ...
    err = clEnqueueNDRangeKernel(sap_gpu_ll_commands, sap_gpu_ll_kernel, 1, NULL, &global, &local, 0, NULL, NULL);
    if (err)
    {
    ...
    clFinish(sap_gpu_ll_commands);
    chronometer.stop();
    if(time_collision)
        *time_collision=chronometer.get_time();
    chronometer.play();
    err = clEnqueueReadBuffer(sap_gpu_ll_commands,output_buffer,CL_TRUE,0,sizeof(char)*max_outputs,gpu_ll_results,0,NULL,NULL);
    if (err != CL_SUCCESS)
    {
...
#endif // SAP_GPU_LL_ITREE_PARALLEL_H_INCLUDED
